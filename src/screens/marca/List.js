...
const List = () => {
  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      const result = await http.get(process.env.REACT_APP_PATH + '/marca/' + page + '/' + state.rowsPerPage);
  ...
  useEffect(() => {
    dispatch({ type: 'title', title: 'Gestión de Marcas - Almacén de BAH' });
    fetchData(state.page)
  }, [state.page, state.rowsPerPage]);

  const createOnClick = () => {
    navigate('/marca/create');
  };

  const editOnClick = () => {
    navigate('/marca/' + selected[0] + '/edit');
  }
  ...
  const deleteOnClick = () => {
    dispatch({
      type: "confirm", msg: 'Esta seguro de eliminar el registro seleccionado?', cb: (e) => {
        if (e) {
          http.delete('/marca/' + selected.join(',')).then((result) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell style={{ minWidth: '80%', maxWidth: '80%' }} className='bg-gore border-table'>Marca
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ minWidth: '20%', maxWidth: '20%' }} className='bg-gore border-table'>Estado
