...
export const Form = () => {
  ...
  useEffect(() => {
    if (pid) {
      if (networkStatus.connected) {
        http.get(process.env.REACT_APP_PATH + '/marca/' + pid).then((result) => {
          set(result);
        });
      }
    } else {
  ...
  const onClickAdd = async () => {
    navigate('/marca/create', { replace: true });
  }
  ...
  const onClickSave = async () => {
    ...
    if (0 || form != null && validate(form)) {
      if (networkStatus.connected) {
        http.post(process.env.REACT_APP_PATH + '/marca', o).then(async (result) => {
          if (!o._id) {
            if (result.id) {
              // navigate('/dependencia/' + result.id + '/edit', { replace: true });
              dispatch({ type: "snack", msg: 'Registro grabado!' });
              navigate('/marca', { replace: true });
            }
            else {
              navigate(-1);
            }
