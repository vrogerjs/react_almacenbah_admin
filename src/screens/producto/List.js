...
const List = () => {
  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      const result = await http.get(process.env.REACT_APP_PATH + '/producto/stock/' + page + '/' + state.rowsPerPage);
  ...
  useEffect(() => {
    dispatch({ type: 'title', title: 'Gestión de Productos - Almacén de BAH' });
    fetchData(state.page)
  ...
  const editOnClick = () => {
    navigate('/producto/' + selected[0] + '/edit');
  }
  ...
  const deleteOnClick = () => {
    dispatch({
      type: "confirm", msg: 'Esta seguro de eliminar el registro seleccionado?', cb: (e) => {
        if (e) {
          http.delete('/producto/' + selected.join(',')).then((result) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell style={{ width: "15%" }} className='bg-gore border-table'>Codigo
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "30%" }} className='bg-gore border-table'>Producto
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "20%" }} className='bg-gore border-table'>Unidad
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "15%" }} className='bg-gore border-table'>Marca
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "10%" }} className='bg-gore border-table'>Stock
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className='border-table' >
                        {row.producto.unidad.cantidad} {row.producto.unidad.abreviatura} ({row.producto.unidad.nombre})
                      </TableCell>
                      <TableCell className='border-table' >
