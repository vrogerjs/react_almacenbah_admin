...
export const Form = () => {
  ...
  const onClickAdd = async () => {
    navigate('/persona/create', { replace: true });
  }
  ...
  const onClickSave = async () => {
    ...
    if (0 || form != null && validate(form)) {
      if (networkStatus.connected) {
        http.post(process.env.REACT_APP_PATH + '/persona', o).then(async (result) => {
          if (!o._id) {
            if (result.id) {
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    ...
                    size="medium"
                    id="standard-name"
                    label="Ingrese el Nro de Celular: "
                    placeholder="Celular"
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position="start">
                    ...
                    {...defaultProps("celular", { required: false })}
                  />
                </Grid>
              </Grid>
