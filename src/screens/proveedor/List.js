...
const List = () => {
  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      const result = await http.get(process.env.REACT_APP_PATH + '/proveedor/' + page + '/' + state.rowsPerPage);
  ...
  useEffect(() => {
    dispatch({ type: 'title', title: 'Gestión de Proveedores - Almacén de BAH' });
    fetchData(state.page)
  ...
  const editOnClick = () => {
    navigate('/proveedor/' + selected[0] + '/edit');
  }
  ...
  const deleteOnClick = () => {
    dispatch({
      type: "confirm", msg: 'Esta seguro de eliminar el registro seleccionado?', cb: (e) => {
        if (e) {
          http.delete('/proveedor/' + selected.join(',')).then((result) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell style={{ width: "35%" }} className='bg-gore border-table'>Proveedor
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "10%" }} className='bg-gore border-table'>RUC
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
