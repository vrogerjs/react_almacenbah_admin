...
const List = () => {
  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      const result = await http.get(process.env.REACT_APP_PATH + '/ingreso/' + page + '/' + state.rowsPerPage);
  ...
  useEffect(() => {
    dispatch({ type: 'title', title: 'Gestión de Ingresos de Productos - Almacén de BAH' });
    fetchData(state.page)
  ...
  const verOnClick = () => {
    navigate('/ingresoproducto/' + selected[0] + '/ver');
  }
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell style={{ width: "20%" }} className='bg-gore border-table'>Procedencia
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "20%" }} className='bg-gore border-table'>Destino
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
                <StyledTableCell style={{ width: "20%" }} className='bg-gore border-table'>Atención
                  {/* <TextField {...defaultProps('dependencia')} style={{ padding: 0, marginTop: '5px !important' }} /> */}
                </StyledTableCell>
